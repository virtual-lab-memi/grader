...
module.exports = {
    attachJob: attachJob,
    attachSingleJob: attachSingleJob
};

function attachSingleJob(taskExecutionID) {
  Logger.info('Attach new job: taskExecutionId=%s', taskExecutionID);
  db.taskExecutions.findOne({_id: taskExecutionID}, function(err, doc) {
    if (err || !doc) {
...
function attachJob(problemId, solutionId, solutionFile, eventIdnpm) {
  var result;
