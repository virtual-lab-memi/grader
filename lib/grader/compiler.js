...
exports.compile = function(data, done) {
  ...
  switch (language) {
    case 'Java':
      logger.info('Starting compilation: %s', language);
      compileJava(data, solutionFile, done);
      break;
    default:
      logger.info('No option available for compilation: %S', language);
...
function compileFinished(data, code, done) {
  ...
  if (code === 0) {
    ...
    logger.info('FINAL RESULT: Not succesful compilation');
